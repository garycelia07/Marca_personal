...
/** GET /api/materials/course/{courseId} — materiales de un curso (paginado). */
export async function GET(request: Request, { params }: Context) {
    ...
    try {
        const response = await backendFetch(`/materials/course/${encodeURIComponent(courseId)}?page=${page}&limit=${limit}`);
        const payload = await response.json().catch(() => null);
        if (!response.ok) {
            const message = payload && typeof payload === "object" && "message" in payload && payload.message
                ? String(payload.message)
                : "No fue posible obtener los materiales del curso.";
