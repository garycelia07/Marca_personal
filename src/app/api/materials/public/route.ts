...
/** GET /api/materials/public?page=1&limit=20 — materiales públicos para la landing. */
export async function GET(request: Request) {
    ...
    try {
        const response = await backendFetch(`/materials/public?page=${page}&limit=${limit}`);
        const payload = await response.json().catch(() => null);
        if (!response.ok) {
            const message = payload && typeof payload === "object" && "message" in payload && payload.message
                ? String(payload.message)
                : "No fue posible obtener los materiales.";
