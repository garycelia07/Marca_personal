    ...
    return (
        <>
            <div className="flex gap-3 overflow-x-auto snap-x snap-mandatory pb-2 sm:grid sm:grid-cols-2 sm:gap-4 lg:grid-cols-5 lg:gap-x-3">
                {shown.map((course) => (
                    <div key={course.id} className="card-pop shrink-0 w-[240px] snap-start flex flex-col overflow-hidden rounded-2xl border hairline bg-[var(--paper)] transition hover:border-[var(--line)]">
                        <button
                            type="button"
